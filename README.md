...
- 개발 기준: UMO 1.1.17. 원본 최신 버전과 별도로 관리합니다.
...
현재 공개 배포판은 원본 UMO 1.1.17 기반 한국어 beta.10과 Windows 소스·빌드 도우미입니다. Android와 PC는 같은 공용 번역 및 검수 완료 이미지 52개를 사용합니다. Windows 독립 실행 빌드, PC용 텍스처 캐시, 그래픽 진단, 키·게임패드 설정 도구, PC 전용 판정 보조·저부하 설정·이동 롱노트 보완이 포함됩니다. 이미지 번역은 일부만 진행됐고 전곡·전 의상·게임패드 검증도 완료되지 않았습니다. [PC 설정 및 검수용 자원 충전 안내](Tools/Windows/PC_SETTINGS.md)를 참고하세요. 검수용 자원 충전은 게임을 종료한 뒤 선택한 PC 원본 세이브에 직접 적용하며, 매번 먼저 백업하고 사용자 확인을 받습니다. 프로필 전환은 필요 없습니다.

Android 추가 데이터 설치용 **한국어 PC 서버 도우미**가 포함됩니다. ZIP 두 개를 지정 폴더에 넣으면 압축 해제, 파일 배치, 네트워크 주소 안내와 서버 시작을 자동으로 처리합니다. 게임 데이터와 PC 패치 자체는 저장소 및 Releases에 포함하지 않습니다. 공식 로그인 보너스 DLC는 Android APK에 내장되어 첫 실행 때 자동 설치·활성화되며, PC판은 빌드 도우미가 원본 UMO 서버에서 검증된 패키지를 받아 자동 설치하고 Windows용으로 변환합니다.
...
1. [Releases](https://github.com/ccs21/UMO_Kor/releases)에서 **Pre-release(베타)**의 `UMO_Kor_For_Android_20260910.zip`을 받아 PC에서 압축을 풉니다. Source code ZIP은 설치 파일이 아닙니다.
...
이미 한국어판 beta.1~beta.9를 사용 중인 경우에는 합본 ZIP을 다시 받을 필요 없이 릴리스의 `UMO_Kor-1.1.17-ko-beta.10.apk`만 받아 기존 앱을 삭제하지 않고 덮어 설치하세요. APK 단독 파일은 신규 설치자의 데이터 서버를 포함하지 않습니다.
...
1. 한국어판 Releases에서 `UMO_Kor_For_Android_20260910.zip`을 받아 쓰기 가능한 새 폴더에 풉니다.
...
명령어를 직접 입력하기 어려운 사용자는 Releases의 `UMO_Kor_For_windows_20260910.zip`을 새 폴더에 풀고, 동봉된 `우타마크로스 오프라인 PC 빌드 도우미 사용방법.pdf`를 읽은 뒤 `UMO_PC_Build_Assistant.exe`를 실행합니다. **도우미가 들어 있는 폴더가 기본 작업 폴더**가 되므로, 쓰기 가능한 빈 폴더에서 시작하고 수십 GB 이상의 여유 공간을 확보하세요.
...
- 번역 문자열 줄바꿈 처리 보완. 검수한 이미지 43개를 Android와 PC에 공통 내장했으며 나머지 이미지 번역은 작업 중.
- Windows 독립 실행 빌드 도우미, 앱 옆 데이터 경로, PC 번들·텍스처·셰이더 보완.
- 공식 로그인 보너스 DLC 자동 설치·활성화. Android는 APK에 내장하고 PC는 빌드 도우미가 내려받아 Windows용 배경으로 변환합니다.
- PC 롱노트 끝 flick에서 끝 타이밍에 키를 놓는 판정 보완과 튜토리얼 안내 리소스 로드 수정. 실제 입력 및 모든 안내 경로는 추가 검증 필요.
- 한국어 PC 서버 도우미 추가. 원본 데이터 ZIP과 PC 패치를 자동 배치하고 LAN IP 안내, 자동 탐색 및 스트리밍 전송을 처리합니다. 게임 데이터 자체는 포함하지 않습니다.
