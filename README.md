...
이번 배포 범위는 Android 한국어 베타 APK와 Windows 개발 소스입니다. Android beta.2에는 수정 번역 519개와 선물 화면 텍스트 보완이 포함됩니다. Windows 독립 실행 빌드, PC용 텍스처 캐시, 그래픽 진단, 키·게임패드 설정 도구, PC 전용 판정 보조·저부하 설정·이동 롱노트 보완이 포함됩니다. 전곡·전 의상·게임패드 검증은 완료되지 않았습니다. [PC 설정 및 검수용 자원 충전 안내](Tools/Windows/PC_SETTINGS.md)를 참고하세요. 검수용 자원 충전은 복사한 PC 테스트 프로필에만 적용합니다.
