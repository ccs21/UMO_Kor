...
현재 공개 배포판은 Android 한국어 beta.7과 Windows 개발 소스·빌드 도우미입니다. `develop` 브랜치에는 원본 UMO 1.1.17 변경 사항도 병합되어 있으며, 다음 한국어판 빌드·실기 검증 후 새 베타에 포함할 예정입니다. Android beta.7에는 수정4까지의 번역, 스테이터스 화면의 한글 폰트 누락 수정 및 의상강화 항목명 축약이 반영됩니다. PC도 같은 공용 번역을 사용합니다. Windows 독립 실행 빌드, PC용 텍스처 캐시, 그래픽 진단, 키·게임패드 설정 도구, PC 전용 판정 보조·저부하 설정·이동 롱노트 보완이 포함됩니다. 전곡·전 의상·게임패드 검증은 완료되지 않았습니다. [PC 설정 및 검수용 자원 충전 안내](Tools/Windows/PC_SETTINGS.md)를 참고하세요. 검수용 자원 충전은 게임을 종료한 뒤 선택한 PC 원본 세이브에 직접 적용하며, 매번 먼저 백업하고 사용자 확인을 받습니다. 프로필 전환은 필요 없습니다.
