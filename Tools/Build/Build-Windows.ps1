param([string]$Unity = 'C:\Program Files\Unity\Hub\Editor\2018.4.36f1\Editor\Unity.exe')
...
foreach ($step in @(@('PrepareResources','windows-prepare.log'), @('BuildRelease','windows-build.log'))) {
    ...
    $process = Start-Process -FilePath $Unity -WindowStyle Hidden -PassThru -ArgumentList @('-batchmode','-quit','-projectPath',('"' + $project + '"'),'-buildTarget','Win64','-executeMethod',('UMOKoreanWindowsBuild.' + $step[0]),'-logFile',('"' + $log + '"'))
    $process.WaitForExit()
...
Write-Output "Build complete: $project\Build\Windows\UMO_Kor\UMO_Kor.exe"
