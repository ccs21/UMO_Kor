...
- 현재 베타: `1.1.16-ko-beta.2`, versionCode `10002`
- Git 태그: `v1.1.16-ko-beta.2`
...
출력: `Unity/Build/Android/UMO_Kor-1.1.16-ko-beta.2.apk`. 준비 로그는 `Logs/android-prepare.log`, 빌드 로그는 `Logs/android-release.log`입니다. 공개 빌드는 `BuildOptions.None`이며 서명 정보가 없으면 중단됩니다. 개발용 `BuildAndroid`/`BuildParallelTest` 결과를 릴리스에 올리지 마세요.
...
.\Tools\Build\Verify-AndroidRelease.ps1 `
  -Apk '.\Unity\Build\Android\UMO_Kor-1.1.16-ko-beta.2.apk' `
