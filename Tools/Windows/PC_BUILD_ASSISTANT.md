...
1. Releases의 `UMO_Kor_For_windows_20260927.zip`을 쓰기 가능한 새 폴더에 풉니다. 바탕 화면이나 여유 공간이 충분한 별도 드라이브를 권장합니다. 동봉된 `우타마크로스 오프라인 PC 빌드 도우미 사용방법.pdf`도 함께 참고하세요.
...
기존 빌드가 있으면 도우미는 Git의 실제 빌드 입력과 핵심 결과물의 SHA-256을 비교합니다. 모두 같으면 Unity 빌드를 생략합니다. 텍스처 캐시도 원본과 결과물의 해시 및 파일 정보를 기록하며, 바뀌거나 손상된 파일만 다시 변환합니다. 게임 코드나 내장 이미지가 변경된 업데이트는 안전을 위해 Unity 빌드가 한 번 필요합니다.
...
마지막 페이지의 `완료` 버튼은 텍스처 변환과 최종 검증이 진행되는 동안 회색으로 비활성화됩니다. 모든 빌드 작업과 검증을 통과하면 활성화되며, 누르면 다음 두 동작을 수행하고 도우미가 종료됩니다.
