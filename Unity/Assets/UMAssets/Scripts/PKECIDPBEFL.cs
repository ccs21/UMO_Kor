...
public class PKECIDPBEFL
{
    public class DDBKLMKKCDL
    {
        ...
        public int OBGBAOLONDD_UniqueId; // 0x18
	...
	public List<PKECIDPBEFL.DDBKLMKKCDL> JPDPFGFMKHK_rankings = new List<PKECIDPBEFL.DDBKLMKKCDL>(); // 0x18
	...
	public List<int> MEGNAIJPBFF_InventoryClosedAt = new List<int>(); // 0x28
	...
	private long DCLLGOECHPA_PidCrypted; // 0x58
	...
	public long IAGGEOPLJIN_LimitAt { get { return AFNACNGGLBD ^ BBEGLBMOBOF_xorl; } set { AFNACNGGLBD = value ^ BBEGLBMOBOF_xorl; } } //0x93A3D8 FEPMDKOJEAA_bgs 0x93A3F4 IHONMIKPHKO_bgs
	public long IKHKMFNIHKH_PlayAt { get { return IPOONAJLNMD ^ BBEGLBMOBOF_xorl; } set { IPOONAJLNMD = value ^ BBEGLBMOBOF_xorl; } } //0x93A414 IHGJDEMGLAF_bgs 0x93A430 JGIIPDMBIBH_bgs
	public long EHGBICNIBKE_player_id { get { return DCLLGOECHPA_PidCrypted ^ BBEGLBMOBOF_xorl; } set { DCLLGOECHPA_PidCrypted = value ^ BBEGLBMOBOF_xorl; } } //0x93A450 OMMOMEDOFNO_bgs 0x93A46C GKBOGMILGJE_bgs
	public int OKJNNGJJOFA_RaidBossId { get { return KNKBLHJABLA ^ FBGGEFFJJHB_xor; } set { KNKBLHJABLA = value ^ FBGGEFFJJHB_xor; } } //0x93A48C NCKKKAHNJJB_bgs 0x93A4A0 AMIPKODLKDN_bgs
	...
	public void DOMFHDPMCCO_Init(List<DDBKLMKKCDL> _JPDPFGFMKHK_rankings, List<string> PNMLDKNKIIM, List<int> CIGDBPLIGKM, List<string> _JIMKNDJMCID_Keys, List<int> _EMEKFFHCHMH_RewardEnd2, bool DCOJMLNFPFM, List<string> _OHNJJIMGKGK_Names, EDOHBJAPLPF_JsonData _NMICBJDPLOH_player, EDOHBJAPLPF_JsonData _LAEMKLPEIAK_Logs, List<int> _BIOBLLHBKBC_Gpgs, long _IKHKMFNIHKH_PlayAt, long _IAGGEOPLJIN_LimitAt, string _ECAOENGDBMI_SaveHash, int _OKJNNGJJOFA_RaidBossId, List<PBJPACKDIIB_NetCoopQuestManager.IFCOFHAFMON> _CFLLEDGILPK_CoopQuest)
	{
		...
		if (_JPDPFGFMKHK_rankings != null)
			JPDPFGFMKHK_rankings.AddRange(_JPDPFGFMKHK_rankings);
		PKIKNGJNJJH_RankKeys.Clear();
		if (PNMLDKNKIIM != null)
			PKIKNGJNJJH_RankKeys.AddRange(PNMLDKNKIIM);
		if (CIGDBPLIGKM != null)
			LMKDNNCAAIJ_RankCloseDate.AddRange(CIGDBPLIGKM);
		if (_JIMKNDJMCID_Keys != null)
			PLMMGCFLFGC_Keys.AddRange(_JIMKNDJMCID_Keys);
		if (_EMEKFFHCHMH_RewardEnd2 != null)
	...
	private EDOHBJAPLPF_JsonData KDMHABGINMG()
	{
		EDOHBJAPLPF_JsonData res = new EDOHBJAPLPF_JsonData();
		{
			...
			if (JPDPFGFMKHK_rankings != null)
			{
				for (int i = 0; i < JPDPFGFMKHK_rankings.Count; i++)
				{
					EDOHBJAPLPF_JsonData data2 = new EDOHBJAPLPF_JsonData();
					data2["id"] = JPDPFGFMKHK_rankings[i].BLJIJENHBPM_Id;
					data2["sc"] = JPDPFGFMKHK_rankings[i].OACABIDEMGG_Score;
					data2["unq"] = JPDPFGFMKHK_rankings[i].OBGBAOLONDD_UniqueId;
					data2["idx"] = JPDPFGFMKHK_rankings[i].NOBCHBHEPNC_Idx;
					data.Add(data2);
				}
			}
			res["rankings"] = data;
		...
		}
		{
			...
			{
				for (int i = 0; i < MEGNAIJPBFF_InventoryClosedAt.Count; i++)
				{
					data.Add(MEGNAIJPBFF_InventoryClosedAt[i]);
				}
			}
			res["closedat"] = data;
		}
		...
		res["raidboss_id"] = OKJNNGJJOFA_RaidBossId;
		{
			...
			if (CFLLEDGILPK_CoopQuest != null)
			{
				for (int i = 0; i < CFLLEDGILPK_CoopQuest.Count; i++)
				{
					...
					data2["dai"] = CFLLEDGILPK_CoopQuest[i].CGHNCPEKOCK_IsDaily;
					data.Add(data2);
				}
			}
			...
			else
			{
				if(json.BBAJPINMOEP_Contains("rankings"))
				{
					...
					for(int i = 0; i < rankings.HNBFOAJIIAL_Count; i++)
					{
						...
						d.OBGBAOLONDD_UniqueId = (int)rankings[i]["unq"];
						d.NOBCHBHEPNC_Idx = (int)rankings[i]["idx"];
						JPDPFGFMKHK_rankings.Add(d);
					}
				}
				...
				if(json.BBAJPINMOEP_Contains("closedat"))
				{
					...
					{
						MEGNAIJPBFF_InventoryClosedAt.Add((int)closedat[i]);
					}
				}
				...
				HJDBGMMPPEF_PlayerData = json["player"];
				if(json.BBAJPINMOEP_Contains("coop_quests"))
				{
					...
					for(int i = 0; i < coop_quests.HNBFOAJIIAL_Count; i++)
					{
						...
						d.CGHNCPEKOCK_IsDaily = JsonUtil.GetInt(coop_quests[i], "dai") == 1;
