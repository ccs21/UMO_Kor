...
namespace ExternLib
{
    public static partial class LibSakasho
    {
		public static void UnlockAll(PlayerData playerData)
		{
				...
				for (int i = 0; i < cnt; i++)
				{
					if (dbEmblem[i].PLALNIIBLOF_en == 2)
					{
						if (saveEmblems[i].BEBJKJKBOGH_date == 0)
						{
							...
							saveEmblems[i].FHCAFLCLGAA_em_cnt = 1;
						}
					}
				}
			}
			{
				...
				for (int i = 0; i < costumeDb.CDENCMNHNGA_table.Count; i++)
				{
					...
					{
						if (costumeBlock.FABAGMLEKIB_CostumeList[i].BEBJKJKBOGH_date == 0)
						{
							costumeBlock.FABAGMLEKIB_CostumeList[i].BEBJKJKBOGH_date = time;
						}
						costumeBlock.FABAGMLEKIB_CostumeList[i].ANAJIAENLNB_lv = costumeDb.CDENCMNHNGA_table[i].LLLCMHENKKN_LevelMax;
					}
				}
				for (int i = 0; i < 30; i++)
				{
			...
			}
			{
				...
				for (int i = 0; i < dbValk.CDENCMNHNGA_table.Count; i++)
				{
					if (dbValk.CDENCMNHNGA_table[i].IPJMPBANBPP_Enabled)
					{
						...
						GKFMJAHKEMA_ValSkill.CCPFGNNIBDD valkSkill = dbValkSkill.MNHBHNIHJJH(dbValk.CDENCMNHNGA_table[i].BMIJDLBGFNP_skill);
						if (valkSkill != null && valkSkill.DOOGFEGEKLG_max > 0)
						{
							valkBlock.CNGNBKNBKGI_ValkList[i].CIEOBFIIPLD_Level = valkSkill.DOOGFEGEKLG_max;
						}
					}
				}
			}
			...
			// Add raid items
			{
				for(int i = 0; i < newData.PJCMHDEJLGF_EventRaidLobby.FBCJICEPLED.Count; i++)
				{
					if(newData.PJCMHDEJLGF_EventRaidLobby.FBCJICEPLED[i].MPCAGEPEJJJ_Key != "")
					{
						for(RaidItemConstants.Type it = RaidItemConstants.Type.FoldRadar; it < RaidItemConstants.Type._maxId; it++)
						{
							if(newData.PJCMHDEJLGF_EventRaidLobby.FBCJICEPLED[i].KAINPNMMAEK_GetItemCount(it) < 99)
								newData.PJCMHDEJLGF_EventRaidLobby.FBCJICEPLED[i].PPJAGFPBFHJ_SetItemCount(it, 99);
						}
					}
				}
			}
		...
		public static bool CheckSaveFile(int userId, string fileName, string error_filename, EDOHBJAPLPF_JsonData replaceIfDiff = null)
		{
			...
			newData_.IIEMACPEEBJ_Deserialize(blocks.Keys.ToList(), IKPIMINCOPI_JsonMapper.PFAMKCGJKKL_ToObject(jsonFile));
