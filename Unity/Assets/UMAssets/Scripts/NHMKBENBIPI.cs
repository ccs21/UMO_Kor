...
public class NHMKBENBIPI_NetDelayBunchInstallManager
{
	...
	public void OFLDICKPNFD(bool CJPFICKPJPP, DJBHIFLHJLK FGGJNGCAFGK)
	{
		if(CJPFICKPJPP)
		{
			MAIHLKPEHJN = JHHBAFKMBDL_NetUIControl.HHCJCDFCLOB_Instance.JDIDDHBIGIO;
		}
		else
		{
	...
	private IEnumerator EOFJPNPFGDM_Coroutine_Install(bool HGEJBINBHIC, IMCBBOAFION _BHFHGFKBOHH_OnSuccess, JFDNPFFOACP NIMPEHIECJH, DJBHIFLHJLK _MOBEEPPKFLG_OnFail)
	{
		...
		JEHIAIPJNJF_FileDownloader.FMOECHMCHPE LBGNKOJFOFC = (JEHIAIPJNJF_FileDownloader.HCJPJKCIBDL_DldFileInfo JGBPLIGAILE) =>
		{
			//0x1893204
			LOBGHGIBFMM = true;
			KEHOJEJMGLJ_NetInstallManager.GFOMKMANCPP(JGBPLIGAILE.ADHHKEMDOIK_LocalPath, JGBPLIGAILE.LAPFOLJGJMB_AssetFileInfo.CALJIGKCAAH_last_updated_at, JGBPLIGAILE.LAPFOLJGJMB_AssetFileInfo.HHPEMFKDHLK_Hash, true);
			KEHOJEJMGLJ_NetInstallManager.HHCJCDFCLOB_Instance.KLIJFOBEKBE.OJCJPCHFPGO_Delete(JGBPLIGAILE.AJPIGKBIDDL_LocalFileName);
			PKKHIEAEDPC d = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.IELDDHJMFKN_Asset.NBHDIKJMLEN(JGBPLIGAILE.AJPIGKBIDDL_LocalFileName);
			if(d != null)
			{
				if(d.NKGJOAEDCPH_rule.PAAPNEMBHGN_Day > 0)
				{
					FECDBKKBAHO.FHOPNIJCFKA_FileInfo f = KEHOJEJMGLJ_NetInstallManager.HHCJCDFCLOB_Instance.KLIJFOBEKBE.ANIJHEBLMGB_SetValue(JGBPLIGAILE.AJPIGKBIDDL_LocalFileName, CPHFEPHDJIB_ServerRequester.FJDBNGEPKHL_Time.KMEFBNBFJHI_GetServerTime(), d.KKPAHLMJKIH_WavId);
		...
		while(true)
		{
			...
			if (JKJCKIMGJLM.CMCKNKKCNDK_status != JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.LPLEIJIFOKN_4_AllError)
			{
				...
				if (prev != BNNHBBCCPDD)
				{
					KEHOJEJMGLJ_NetInstallManager.HHCJCDFCLOB_Instance.KLIJFOBEKBE.HJMKBCFJOOH_Save();
					KEHOJEJMGLJ_NetInstallManager.JKIBGMKGMCK(true);
				}
	...
	private IEnumerator HKBHCEBAACH_Co_Popup(Action AGPNLAKFKCN, Action NIMPEHIECJH)
	{
		...
		if(ICCMKHKNAMJ_ToDldList.Count > 0)
		{
			LFPOMKLKHPB_InstallFileSize AODHFMGKPNJ = new LFPOMKLKHPB_InstallFileSize();
	...
	public static List<GCGNICILKLD_AssetFileInfo> IAPEABPJPOE(string _OGCDNCDMLCA_LocalPath, IKAHKDKIGNA CBLEBKOJJDB, List<string> _JOJMBFBGMGN_DldQueue)
	{
		...
		{
			PKKHIEAEDPC dbFile = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.IELDDHJMFKN_Asset.NBHDIKJMLEN(CBLEBKOJJDB.KGHAJGGMPKL_files[i].OIEAICNAMNB_LocalFileName);
			bool b = false;
			...
			if(b)
			{
				...
				for (int k = 0; k < l3.Count; k++)
				{
					...
					{
						if (KEHOJEJMGLJ_NetInstallManager.NHIIAHGHOMH(s, CBLEBKOJJDB.KGHAJGGMPKL_files[i].CALJIGKCAAH_last_updated_at, CBLEBKOJJDB.KGHAJGGMPKL_files[i].HHPEMFKDHLK_Hash))
						{
	...
	{
		foreach (var k in IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.IELDDHJMFKN_Asset.OAOOIDNBOGA_MVer_List)
		{
