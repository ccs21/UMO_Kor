	...
	namespace CriMana
	{
		public class Player : CriDisposable
		{
			...
			public void SetMasterTimerType(Player.TimerType timerType)
			{
				...
				CRIWARE31D7EFF8(playerId, timerType);
			}
			...
			public void Update()
			{
				if(_timerType == TimerType.Manual || _timerType == TimerType.User)
					return;
				InternalUpdate();
			}
...
#else
					UnityEngine.GL.IssuePluginEvent(criWareUnity_GetRenderEventFunc(), this.playerId | (int)CriManaPlugin.renderingEventOffset | (int)renderEventAction);
#endif
				}
#endif
#endif
			...
			private void InternalUpdate()
			{
				...
				switch(_nativeStatus)
				{
					...
					{
#if UNITY_ANDROID
						CRIWARE4A28D964_criManaUnityPlayer_GetMovieInfo(playerId, _movieInfo);
#else
			...
			private IEnumerator IssuePluginUpdatesForFrames(int frameCount, MonoBehaviour playerHolder, bool destroy, int playerId)
			{
				...
				while(frameCount >= 1)
				{
				UnityEngine.Debug.LogError("IssuePluginUpdatesForFrames");
					IssuePluginEvent(CriManaUnityPlayer_RenderEventAction.UPDATE);
					CRIWARE55BA8D00(playerId);
					frameCount --;
					yield return null;
			...
			{
#if UNITY_ANDROID
				CRIWARE4A28D964(player_id, movie_info);
#else
...
#else
			private static void CRIWARE31D7EFF8(int player_id, Player.TimerType timer_type)
			{
